...
const splits = [
  { name: 'Push / Pull / Legs', meta: 'Classic 6 day split', accent: 'Chest • Back • Legs', days: '6 days', path: '/exercise?split=ppl' },
  { name: 'Upper / Lower', meta: '4 day balanced split', accent: 'Strength + balance', days: '4 days', path: '/exercise?split=upper-lower' },
  { name: 'Bro Split', meta: 'Chest / Back / Arms / Legs', accent: 'High volume pump', days: '5 days', path: '/exercise?split=bro' },
  { name: 'Full Body', meta: 'Total body 3x per week', accent: 'Beginner base', days: '3 days', path: '/exercise?split=full-body' },
  { name: 'Athletic', meta: 'Strength + conditioning', accent: 'Explosive work', days: '5 days', path: '/exercise?split=athletic' },
  { name: 'Calisthenics', meta: 'Bodyweight strength', accent: 'No excuses', days: '4 days', path: '/exercise?split=calisthenics' },
  { name: 'Recovery', meta: 'Low impact, heal & rebuild', accent: 'Mobility + zone 2', days: '3 days', path: '/exercise?split=recovery' },
  { name: 'Custom Split', meta: 'Build your own week', accent: 'Pick muscles', days: 'Custom', path: '/exercise?split=custom' },
];
...
export function HelmetCoach({ small = false }: { small?: boolean }) {
  return (
    <div className={small ? 'ih-coach ih-coach-small' : 'ih-coach'} aria-label="Iron helmet hoodie coach">
      ...
      <div className="ih-hood">
        <div className="ih-helmet">
          <span className="ih-ridge" />
          <span className="ih-slit" />
          <span className="ih-breath" />
        </div>
        <div className="ih-shoulders" />
      </div>
    </div>
  );
}
...
function MediaTile({ label, tall = false }: { label: string; tall?: boolean }) {
  return (
    <div className={tall ? 'ih-media ih-media-tall' : 'ih-media'}>
      <div className="ih-media-grid" />
...
export function TrainPage() {
  return (
    <section className="ih-page">
      ...
      <div className="ih-list">
        {splits.map((split) => <Link className="ih-split" to={split.path} key={split.name}><MediaTile label="GIF" /><div><strong>{split.name}</strong><small>{split.meta}</small><ProgressBar pct={split.name === 'Recovery' ? 45 : 70} /></div><em>{split.days}</em><b>›</b></Link>)}
      </div>
...
export function ExerciseDetail() {
  return (
    <section className="ih-page">
      ...
      <h2>EXERCISES</h2>
      <div className="ih-list">{exercises.map((exercise, index) => <Link className="ih-exercise" to="/workout-mode" key={exercise.name}><MediaTile label="DEMO" /><div><strong>{index + 1} {exercise.name}</strong><small>{exercise.sets}</small><em>{exercise.muscle}</em></div><b>▶</b></Link>)}</div>
      <div className="ih-card"><h2>BARBELL BENCH PRESS</h2><MediaTile label="DEMO GIF / VIDEO AREA" tall /><div className="ih-tabs"><b>DEMO</b><span>MUSCLES</span><span>CUES</span></div><ul className="ih-cues"><li>Grip slightly wider than shoulders.</li><li>Retract shoulder blades.</li><li>Lower bar to mid-chest.</li><li>Press up fast. No ego.</li></ul><div className="ih-muscles">Target muscles: Chest, front delts, triceps</div></div>
...
export function WorkoutLogger() {
  return (
    <section className="ih-page">
      ...
      <MediaTile label="ACTIVE DEMO GIF / VIDEO" tall />
