...
const defaultCards = buildTrainingProgramCards(null, { getDay: () => 1 });
...
const loadedCards = buildTrainingProgramCards(activeLoadout, { getDay: () => 2 });
...
assert.equal(getWorkoutSplitSelection(activeLoadout, 'legs').family.id, 'ppl');
assert.equal(getWorkoutSplitSelection(activeLoadout, 'legs').day.name, 'Legs');
assert.equal(getWorkoutSplitSelection({ ...activeLoadout, splitFamilyId: 'arnold' }, 'legs').family.id, 'arnold');
